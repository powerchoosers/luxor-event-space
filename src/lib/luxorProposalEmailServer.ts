...
export function buildLuxorPaymentRequestEmail(input: {
  ...
  return {
    ...
    html: `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width,initial-scale=1.0" /><title>Luxor Payment Request</title></head>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#050505">
    <tr><td align="center" style="padding:28px 16px;">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#0a0807;border:1px solid rgba(202,162,76,0.22);border-radius:4px;overflow:hidden;">
        ...
        <tr><td style="padding:28px 48px 20px;text-align:center;background-color:#080605;border-bottom:1px solid rgba(202,162,76,0.14);">
        ...
        <tr><td style="padding:52px 48px 32px;text-align:center;background:radial-gradient(circle at 50% 0%,rgba(202,162,76,0.18),transparent 70%),linear-gradient(180deg,#120d0a,#050505);">
          <p style="margin:0 0 16px;font-size:10px;font-weight:700;letter-spacing:0.34em;text-transform:uppercase;color:#caa24c;">Proposal & Payment Request</p>
          <h1 style="margin:0 0 16px;font-family:Georgia,'Times New Roman',serif;font-size:38px;font-weight:600;line-height:1.08;color:#f7efe3;">Your Event Proposal Is Ready</h1>
          <p style="margin:0 auto;max-width:460px;font-size:15px;line-height:1.8;color:rgba(215,194,154,0.82);">Hi ${escapeHtml(firstName)}, your Luxor proposal is attached. Review the services below, then use the secure button to make your ${escapeHtml(paymentLabel.toLowerCase())}.</p>
        </td></tr>
        ...
        <tr><td style="padding:34px 48px 12px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
            <tr>
              <td width="50%" style="vertical-align:top;padding-right:16px;border-right:1px solid rgba(202,162,76,0.18);"><p style="margin:0 0 8px;font-size:9px;font-weight:700;letter-spacing:0.28em;text-transform:uppercase;color:#caa24c;">Event</p><p style="margin:0;font-size:14px;color:#f7efe3;">${escapeHtml(invoice.event_type || 'Private Event')}</p></td>
              <td width="50%" style="vertical-align:top;padding-left:20px;"><p style="margin:0 0 8px;font-size:9px;font-weight:700;letter-spacing:0.28em;text-transform:uppercase;color:#caa24c;">Event Date</p><p style="margin:0;font-size:14px;color:#f7efe3;">${escapeHtml(inquiry.target_date || 'To be confirmed')}</p></td>
            </tr>
          </table>
        </td></tr>
        ...
        <tr><td style="padding:24px 48px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#0d0b09;border:1px solid rgba(202,162,76,0.18);">
            <tr><td style="padding:18px 20px;font-size:12px;color:rgba(215,194,154,0.68);">Proposal total</td><td align="right" style="padding:18px 20px;font-size:13px;color:#f7efe3;">${money(invoice.total)}</td></tr>
            <tr><td style="padding:0 20px 18px;font-size:12px;color:rgba(215,194,154,0.68);">Previously paid</td><td align="right" style="padding:0 20px 18px;font-size:13px;color:#f7efe3;">${money(paidTotal)}</td></tr>
            ...
            ${remainingAfterPayment > 0 ? `<tr><td style="padding:0 20px 18px;font-size:11px;color:rgba(215,194,154,0.54);">Remaining after this payment</td><td align="right" style="padding:0 20px 18px;font-size:12px;color:rgba(215,194,154,0.72);">${money(remainingAfterPayment)}</td></tr>` : ''}
          </table>
        </td></tr>
        <tr><td align="center" style="padding:8px 48px 42px;">
          ...
          <p style="margin:18px 0 0;font-size:11px;line-height:1.7;color:rgba(215,194,154,0.48);">Payments are processed securely by Stripe. Reply to this email before paying if any service or quantity needs to change.</p>
        </td></tr>
