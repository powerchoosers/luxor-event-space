...
function offerDisclosureHtml(invoice: LuxorInvoice) {
...
export async function buildLuxorPaymentRequestEmail(input: {
  ...
  const { invoice, inquiry, booking, reviewUrl, paymentAmount, paymentLabel, paidTotal, balanceDue } = input
  ...
  const paymentScheduleNote = invoice.invoice_kind === 'final_balance'
    ? `This payment includes the remaining event balance and the ${money(Number(booking.security_deposit_amount ?? 750))} refundable security deposit, due ${finalDueDate}.`
    : `This reservation deposit is due when the agreement is signed. The remaining event balance and the ${money(Number(booking.security_deposit_amount ?? 750))} refundable security deposit are due ${finalDueDate}.`
