...
export async function POST(request: NextRequest) {
  ...
  const [member] = await supabaseRest<LuxorPortalMember[]>('luxor_portal_members?on_conflict=email&select=*', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=representation' }, body: JSON.stringify({ email, display_name: displayName, role, status: 'pending', permissions, sender_email: cleanEmail(body.senderEmail), assigned_phone_number_id: typeof body.phoneNumberId === 'string' ? body.phoneNumberId : null, updated_at: new Date().toISOString() }) })
...
export async function PATCH(request: NextRequest) {
  ...
  const [member] = await supabaseRest<LuxorPortalMember[]>(`luxor_portal_members?id=eq.${id}&select=*`, { method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ display_name: String(body.displayName || current[0].display_name).trim().slice(0, 100), role, status, permissions: normalizePermissions(body.permissions, role), sender_email: cleanEmail(body.senderEmail), assigned_phone_number_id: typeof body.phoneNumberId === 'string' ? body.phoneNumberId : null, updated_at: new Date().toISOString() }) })
