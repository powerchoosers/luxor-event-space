...
const slides = {
  ...
  es: [
    { title: 'Salón principal', subtitle: 'Hasta 180 invitados', image: '/images/dining-hall/main-hall-wedding-wide.png' },
...
const galleryCopy = {
  en: { title: 'Your Tour at a Glance', book: 'Book a Tour', label: 'Tour spaces. Swipe left or right to browse.', previous: 'Previous tour space', next: 'Next tour space', show: 'Show' },
  es: { title: 'Tu recorrido de un vistazo', book: 'Reservar recorrido', label: 'Espacios del recorrido. Desliza hacia la izquierda o derecha para explorar.', previous: 'Espacio anterior', next: 'Espacio siguiente', show: 'Mostrar' },
} as const
...
export function TourGallery({ locale = 'en' }: { locale?: Locale }) {
  ...
  return <section aria-labelledby="tour-gallery-title" className="min-w-0">
    ...
    >
      ...
      >
        {gallerySlides.map((slide, index) => <div key={slide.title} className="relative h-full w-full shrink-0" aria-hidden={index !== active}>
