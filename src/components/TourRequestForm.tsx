  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-[#b98a3d]/30 bg-white p-5 shadow-[0_25px_80px_-44px_rgba(56,38,20,0.45)] sm:p-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        <Field label={spanish ? 'Tipo de evento' : 'Event type'}><select value={eventType} onChange={(event) => setEventType(event.target.value)} className={inputClass} required><option value="">{spanish ? 'Selecciona una opción' : 'Select an event'}</option>{LUXOR_EVENT_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}</select></Field>
        <Field label={spanish ? 'Invitados estimados' : 'Estimated guests'}><input value={guestCount} onChange={(event) => setGuestCount(event.target.value)} inputMode="numeric" placeholder="For example, 120" className={inputClass} /></Field>
        <Field label={spanish ? 'Fecha preferida' : 'Preferred date'}><select value={targetDate} onChange={(event) => changeDate(event.target.value)} className={inputClass}><option value="">{slots.length ? (spanish ? 'Selecciona una fecha' : 'Choose a date') : (spanish ? 'Indica una fecha' : 'Tell us a date')}</option>{dates.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field>
        <Field label={spanish ? 'Hora preferida' : 'Preferred time'}>{slots.length ? <select value={slotId} onChange={(event) => setSlotId(event.target.value)} disabled={!targetDate || slotsLoading} className={inputClass}><option value="">{targetDate ? (spanish ? 'Selecciona una hora' : 'Choose a time') : (spanish ? 'Primero elige una fecha' : 'Choose a date first')}</option>{dateSlots.map((slot) => <option key={slot.id} value={slot.id}>{slot.time}</option>)}</select> : <select value={tourWindow} onChange={(event) => setTourWindow(event.target.value)} className={inputClass}><option value="">{spanish ? 'Selecciona una ventana' : 'Choose a window'}</option>{WINDOWS.map(([value, label]) => <option key={value} value={value}>{spanish && value === 'Flexible' ? 'Soy flexible' : label}</option>)}</select>}</Field>
        <Field label={spanish ? 'Idioma del recorrido' : 'Tour language'}><select value={tourLanguage} onChange={(event) => setTourLanguage(event.target.value as Locale | 'none')} className={inputClass}><option value="en">English</option><option value="es">Español</option><option value="none">{spanish ? 'Sin preferencia' : 'No preference'}</option></select></Field>
