  ...
  return (
    <form id="tour-booking" onSubmit={submit} className="scroll-mt-28 rounded-2xl border border-[#b98a3d]/30 bg-white p-5 shadow-[0_25px_80px_-44px_rgba(56,38,20,0.45)] sm:scroll-mt-32 sm:p-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <Field label={spanish ? 'Fecha del recorrido' : 'Tour date'}><PortalDatePicker theme="light" value={targetDate} onChange={changeDate} minDate={new Date().toISOString().slice(0, 10)} availableDates={customMode ? undefined : availableDates} placeholder={spanish ? 'Indica una fecha' : 'Choose a date'} className={publicDatePickerClass} /><span className="mt-2 block text-[11px] text-[#827567]">{customMode ? (spanish ? 'Elige la fecha que prefieras.' : 'Choose your preferred date.') : (spanish ? 'Los puntos verdes muestran las fechas disponibles.' : 'Green dots show available tour dates.')}</span></Field>
        {!customMode ? <Field label={spanish ? 'Hora del recorrido' : 'Tour time'}><PortalSelect theme="light" value={slotId} onChange={setSlotId} disabled={!targetDate || slotsLoading} options={dateSlots.map((slot) => ({ value: slot.id, label: `${slot.time} · ${spanish ? 'disponible' : 'available'}` }))} placeholder={targetDate ? (spanish ? 'Selecciona una hora' : 'Choose an available time') : (spanish ? 'Primero elige una fecha' : 'Choose a date first')} className="w-full" buttonClassName="min-h-12 rounded-lg bg-[#fffdfa] px-3 text-sm normal-case tracking-normal" /><span className="mt-2 block text-[11px] text-[#827567]">{dateSlots.length ? (spanish ? 'Horarios disponibles para esta fecha.' : 'Available times for this date.') : (spanish ? 'Primero selecciona una fecha con un punto verde.' : 'Select a green-dot date first.')}</span></Field> : <Field label={spanish ? 'Hora preferida' : 'Preferred time'}><input type="time" value={customTourTime} onChange={(event) => setCustomTourTime(event.target.value)} className={inputClass} /></Field>}
        <Field label={spanish ? 'Idioma del recorrido' : 'Tour language'}><PortalSelect theme="light" value={tourLanguage} onChange={(value) => setTourLanguage(value as Locale | 'none')} options={[{ value: 'en', label: spanish ? 'Inglés' : 'English' }, { value: 'es', label: 'Español' }, { value: 'none', label: spanish ? 'Sin preferencia' : 'No preference' }]} className="w-full" buttonClassName="min-h-12 rounded-lg bg-[#fffdfa] px-3 text-sm normal-case tracking-normal" /></Field>
      ...
      <AnimatePresence initial={false} mode="wait">
        {!customMode ? <motion.div key="custom-time-prompt" initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2, ease: [0.23, 1, 0.32, 1] }}>
          ...
          <div className="mt-6 rounded-xl border border-[#b98a3d]/20 bg-[#fbf7ef] p-4"><p className="text-sm font-semibold text-[#241d17]">{spanish ? 'Solicita otra hora' : 'Request a different time'}</p><p className="mt-1 text-xs text-[#665a4e]">{spanish ? 'El equipo de Luxor revisará esta solicitud.' : 'This request is reviewed by the Luxor team.'}</p><div className="mt-4"><Field label={spanish ? 'Fecha alternativa (opcional)' : 'Optional alternate date'}><PortalDatePicker theme="light" value={alternateDate} onChange={setAlternateDate} minDate={new Date().toISOString().slice(0, 10)} placeholder={spanish ? 'Elige otra fecha' : 'Choose another date'} className={publicDatePickerClass} /></Field></div><button type="button" onClick={() => { setCustomMode(false); setCustomTourTime(''); setAlternateDate('') }} aria-expanded="true" className="mt-3 text-xs font-semibold text-[#8d672b] underline underline-offset-4">{spanish ? 'Elegir un horario disponible' : 'Choose an available slot instead'}</button></div>
        </motion.div>}
      </AnimatePresence>
      {slotsError ? <p className="mt-4 text-sm text-rose-700">{spanish ? 'No pudimos cargar los horarios disponibles. Inténtalo de nuevo.' : slotsError}</p> : null}
      ...
      <button type="submit" disabled={submitting} className="mt-7 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-lg bg-[#b98a3d] px-5 text-sm font-bold uppercase tracking-[0.12em] !text-white transition hover:bg-[#9d722e] disabled:cursor-not-allowed disabled:opacity-60">{submitting ? <Loader2 size={16} className="animate-spin" /> : null}{submitting ? (spanish ? 'Enviando…' : 'Sending…') : customMode ? (spanish ? 'Solicitar recorrido' : 'Request Tour') : (spanish ? 'Reservar recorrido' : 'Book a Tour')}</button>
    </form>
  )
}
