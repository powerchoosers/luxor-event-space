...
import { ArrowLeft, CheckCheck, Images, Info, Loader2, MessageSquare, MoreVertical, Phone, Plus, Search, Send, UserPlus, UserRound, X } from 'lucide-react'
...
export function LuxorMessenger() {
  ...
  return (
    <section className="relative grid min-h-[calc(100dvh-9.5rem)] overflow-hidden rounded-2xl border border-[color:var(--portal-border)] bg-[#080807] shadow-2xl lg:grid-cols-[22rem_minmax(0,1fr)]">
      <aside className={`${showThreadOnMobile ? 'hidden lg:flex' : 'flex'} min-h-0 flex-col border-r border-[color:var(--portal-border)]`}>
        ...
        <div className="portal-scrollbar min-h-0 flex-1 overflow-y-auto">
          {loading ? <LoadingConversations /> : visibleConversations.length ? visibleConversations.map((conversation) => (
            <button key={conversation.key} type="button" onClick={() => selectConversation(conversation)} className={`flex w-full gap-3 border-b border-zinc-900/70 px-4 py-4 text-left transition-colors ${selectedConversation?.key === conversation.key ? 'bg-[#caa24c]/8 shadow-[inset_2px_0_0_#caa24c]' : 'hover:bg-white/[0.025]'}`}>
          ...
          <>
            <header className="flex min-h-20 items-center justify-between gap-4 border-b border-[color:var(--portal-border)] px-4 py-3 sm:px-5">
              <div className="flex min-w-0 items-center gap-3">
                <button type="button" onClick={() => setShowThreadOnMobile(false)} className="rounded-lg p-2 text-zinc-500 hover:bg-white/5 hover:text-white lg:hidden" aria-label="Back to conversations"><ArrowLeft size={18}/></button>
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <div className="relative">
                  <button type="button" onClick={() => setOptionsOpen((current) => !current)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-800 text-zinc-500 hover:text-white" aria-label="Conversation options" aria-expanded={optionsOpen}><MoreVertical size={15}/></button>
                  <AnimatePresence>{optionsOpen ? <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute right-0 top-11 z-30 w-56 rounded-xl border border-zinc-800 bg-[#11110f] p-1.5 shadow-2xl">
                    <button type="button" onClick={() => { setOptionsOpen(false); setMediaGalleryOpen(true) }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-left text-[10px] font-bold text-zinc-300 hover:bg-white/5"><Images size={14}/> Media gallery <span className="ml-auto font-mono text-zinc-600">{sharedMedia.length}</span></button>
                    {selectedConversation.inquiryId ? <Link href={`/portal/leads/${selectedConversation.inquiryId}`} onClick={() => setOptionsOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-[10px] font-bold text-zinc-300 hover:bg-white/5"><Info size={14}/> Contact details</Link> : null}
                    <div className="my-1 h-px bg-zinc-800"/>
                    <button type="button" disabled className="flex w-full cursor-not-allowed items-center gap-2 rounded-lg px-3 py-2.5 text-left text-[10px] font-bold text-zinc-600"><UserPlus size={14}/> Add a member</button>
      ...
      <AnimatePresence>{newMessageOpen ? <NewMessageOverlay query={contactQuery} results={contactResults} searching={searchingContacts} onQueryChange={setContactQuery} onSelect={startNewConversation} onClose={() => { setNewMessageOpen(false); setContactQuery('') }} /> : null}</AnimatePresence>
...
function NewMessageOverlay({ query, results, searching, onQueryChange, onSelect, onClose }: { query: string; results: ContactResult[]; searching: boolean; onQueryChange: (value: string) => void; onSelect: (contact: ContactResult) => void; onClose: () => void }) {
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-40 flex items-start justify-center bg-black/70 p-4 pt-16 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <motion.div initial={{ opacity: 0, y: 18, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.98 }} transition={{ duration: 0.28, ease: [0.23, 1, 0.32, 1] }} className="w-full max-w-lg overflow-hidden rounded-2xl border border-[#caa24c]/20 bg-[#10100e] shadow-[0_30px_90px_rgba(0,0,0,0.75)]">
      <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4"><div><p className="text-sm font-black text-white">New message</p><p className="mt-1 text-[10px] text-zinc-600">Search Luxor leads by name or phone number.</p></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-zinc-500 hover:bg-white/5 hover:text-white" aria-label="Close new message"><X size={16}/></button></div>
      <div className="p-4">
        <label className="flex h-11 items-center gap-2 rounded-xl border border-zinc-800 bg-black/40 px-3 focus-within:border-[#caa24c]/45"><Search size={15} className="text-zinc-600"/><input autoFocus value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="Type a name or phone number" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-zinc-700"/>{searching ? <Loader2 size={14} className="animate-spin text-[#caa24c]"/> : null}</label>
        <div className="portal-scrollbar mt-3 max-h-80 overflow-y-auto">
          {query.trim().length < 2 ? <p className="py-10 text-center text-xs text-zinc-650">Enter at least two letters or digits.</p> : searching ? <p className="py-10 text-center text-xs text-zinc-650">Searching Luxor contacts…</p> : results.length ? results.map((contact) => <button key={contact.id} type="button" onClick={() => onSelect(contact)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left hover:bg-white/[0.04]"><PortalContactAvatar name={contact.fullName} inquiryId={contact.id} size="md"/><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-white">{contact.fullName}</span><span className="mt-0.5 block font-mono text-[10px] text-zinc-600">{formatPhoneDisplay(contact.phoneNumber)}{contact.eventType ? ` · ${contact.eventType}` : ''}</span></span><MessageSquare size={15} className="text-[#caa24c]"/></button>) : <p className="py-10 text-center text-xs text-zinc-650">No matching contacts with a mobile number.</p>}
        </div>
      </div>
    </motion.div>
  </motion.div>
