          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="relative">
                <button type="button" onClick={() => setLineOptionsOpen((current) => !current)} className="flex h-9 w-9 items-center justify-center rounded-lg text-zinc-600 transition-colors hover:bg-white/[0.08] hover:text-zinc-100" aria-label="Message line options" aria-expanded={lineOptionsOpen}><MoreVertical size={16}/></button>
                <AnimatePresence>{lineOptionsOpen ? <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute right-0 top-11 z-30 w-64 rounded-xl border border-zinc-800 bg-[#11110f] p-1.5 shadow-2xl">
                  <p className="px-3 pb-1 pt-2 font-mono text-[8px] uppercase tracking-widest text-zinc-600">Show messages for</p>
                  {[{ value: 'all', label: 'All Luxor numbers' }, ...businessLines.map((line) => ({ value: line.phoneNumber, label: `${line.isActive ? 'Main · ' : ''}${formatPhoneDisplay(line.phoneNumber)}` }))].map((option) => <button key={option.value} type="button" onClick={() => { setSelectedLine(option.value); setSelectedKey(null); setDraftConversation(null); setLineOptionsOpen(false) }} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[10px] font-bold text-zinc-300 hover:bg-white/[0.06] hover:text-white"><span className="flex h-4 w-4 items-center justify-center">{selectedLine === option.value ? <Check size={13} className="text-[#caa24c]"/> : null}</span><span>{option.label}</span></button>)}
          ...
          <>
            <header className="flex min-h-20 items-center justify-between gap-4 border-b border-[color:var(--portal-border)] px-4 py-3 sm:px-5">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <div className="relative">
                  <button type="button" onClick={() => setOptionsOpen((current) => !current)} className="flex h-9 w-9 items-center justify-center rounded-lg text-zinc-600 transition-colors hover:bg-white/[0.08] hover:text-zinc-100" aria-label="Conversation options" aria-expanded={optionsOpen}><MoreVertical size={16}/></button>
...
function NewMessagePane({ query, results, searching, onQueryChange, onSelect, onClose }: { query: string; results: ContactResult[]; searching: boolean; onQueryChange: (value: string) => void; onSelect: (contact: ContactResult) => void; onClose: () => void }) {
  return <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.28, ease: [0.23, 1, 0.32, 1] }} className="flex min-h-0 flex-1 flex-col">
    <header className="flex min-h-20 items-center gap-3 border-b border-[color:var(--portal-border)] px-4 py-3 sm:px-5">
      <button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-lg text-zinc-500 transition-colors hover:bg-white/[0.08] hover:text-white" aria-label="Close new message"><ArrowLeft size={18}/></button>
