...
export function TeamAccessManager() {
  ...
  return <div className="space-y-5">
    ...
    {drawerOpen && typeof document !== 'undefined' ? createPortal(<div className="fixed inset-0 z-[100] flex items-end bg-black/45 p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:items-stretch sm:justify-end sm:p-0" onMouseDown={() => { setAdding(false); setEditing(null) }}><section role="dialog" aria-modal="true" aria-label="Manage team access" onMouseDown={(event) => event.stopPropagation()} className="max-h-[calc(100dvh-1.5rem-env(safe-area-inset-bottom))] w-full overflow-y-auto rounded-2xl border border-[color:var(--portal-border)] bg-[color:var(--portal-card)] p-5 shadow-2xl sm:h-full sm:max-h-none sm:w-[min(32rem,calc(100vw-1rem))] sm:rounded-none">
      <div className="flex items-start justify-between"><div><h4 className="text-lg font-bold text-[color:var(--portal-text)]">{adding ? 'Add team member' : ownerReadOnly ? 'Owner profile' : 'Manage access'}</h4><p className="mt-1 text-xs text-[color:var(--portal-muted)]">{adding ? 'They will remain pending until you send an invitation.' : form.email}</p></div><button type="button" aria-label="Close" onClick={() => { setAdding(false); setEditing(null) }} className="rounded-lg p-2 text-[color:var(--portal-muted)] hover:bg-[color:var(--portal-soft)]"><X size={18} /></button></div>
      {ownerReadOnly ? <div className="mt-6 rounded-xl border border-[color:var(--portal-border)] bg-[color:var(--portal-soft)] p-4"><p className="text-sm font-semibold text-[color:var(--portal-text)]">{editing?.display_name}</p><p className="mt-1 text-xs text-[color:var(--portal-muted)]">Owner accounts keep their protected access. Use the profile section above to change this owner’s name, photo, and email signature.</p><button type="button" onClick={() => setEditing(null)} className="mt-5 w-full rounded-lg bg-[#caa24c] px-4 py-3 text-xs font-bold text-white">Close</button></div> : <div className="mt-6 space-y-4"><label className="block text-xs font-semibold text-[color:var(--portal-text)]">Name<input value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} className="mt-2 w-full rounded-lg border border-[color:var(--portal-border)] bg-[color:var(--portal-soft)] px-3 py-2.5 text-sm outline-none focus:border-[#caa24c]" /></label>{adding ? <label className="block text-xs font-semibold text-[color:var(--portal-text)]">Email<input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} className="mt-2 w-full rounded-lg border border-[color:var(--portal-border)] bg-[color:var(--portal-soft)] px-3 py-2.5 text-sm outline-none focus:border-[#caa24c]" /></label> : null}
