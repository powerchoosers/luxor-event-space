...
import { CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, Clock3, Loader2 } from 'lucide-react'
...
export function TourAvailabilityManager({
...
}: { title?: string; description?: string; publishLabel?: string; onUpdated?: () => void | Promise<void>; defaultExpanded?: boolean } = {}) {
  ...
  return <div className={`space-y-5 [&>div.grid]:transition-[max-height,opacity] [&>div.grid]:duration-300 [&>div.grid]:ease-out ${expanded ? '[&>div.grid]:max-h-[80rem] [&>div.grid]:opacity-100' : '[&>div.grid]:max-h-0 [&>div.grid]:overflow-hidden [&>div.grid]:opacity-0'}`}>
    <section className="rounded-xl border border-[color:var(--portal-border)] bg-[color:var(--portal-soft)] p-5">
      <button type="button" aria-expanded={expanded} onClick={() => setExpanded((current) => !current)} className="flex w-full flex-col gap-3 text-left sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0"><span className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-[color:var(--portal-text)]"><ChevronDown size={15} className={`transition-transform duration-300 ${expanded ? 'rotate-180' : ''}`} />{title}</span><p className="mt-2 max-w-2xl text-xs leading-5 text-[color:var(--portal-muted)]">{description}</p></div>
        <div className="shrink-0 rounded-lg border border-[color:var(--portal-border)] bg-[color:var(--portal-card)] px-4 py-3 sm:text-right"><p className="text-[9px] font-bold uppercase tracking-[0.15em] text-[color:var(--portal-faint)]">Public schedule</p><p className="mt-1 text-xs font-semibold text-[color:var(--portal-text)]">{schedule.filter((day) => day.is_open).map((day) => LUXOR_WEEKDAY_LABELS[day.weekday].slice(0, 3)).join(' · ') || 'No days selected'}</p><p className="mt-1 text-[10px] text-[color:var(--portal-muted)]">{openTimes} time{openTimes === 1 ? '' : 's'} per week</p></div>
      </button>
      <div className={`grid transition-[grid-template-rows,opacity,margin] duration-300 ease-out ${expanded ? 'mt-5 grid-rows-[1fr] opacity-100' : 'mt-0 grid-rows-[0fr] opacity-0'}`}>
        <div className="min-h-0 overflow-hidden">
      <div className="space-y-2">{schedule.map((day) => <div key={day.weekday} className={`grid gap-3 rounded-lg border p-3 sm:grid-cols-[minmax(10rem,1fr)_8rem_8rem_auto] sm:items-center sm:gap-x-5 ${day.is_open ? 'border-[#caa24c]/35 bg-[#caa24c]/[0.06]' : 'border-[color:var(--portal-border)] bg-[color:var(--portal-card)]'}`}>
        ...
        <span className="flex items-center gap-1 text-[10px] text-[color:var(--portal-muted)]"><Clock3 size={13} />{day.is_open ? `${tourTimesForAvailability(day).length} slots` : 'Not offered'}</span>
      </div>)}</div>
    ...
    <div style={{ maxHeight: expanded ? '80rem' : '0px', opacity: expanded ? 1 : 0, overflow: 'hidden', transition: 'max-height 300ms ease, opacity 300ms ease' }}>
