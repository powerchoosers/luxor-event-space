...
export function LuxorTextThread({ inquiryId, phone, contactName }: { inquiryId?: string; phone?: string | null; contactName?: string | null }) {
  ...
  return <section className="rounded-2xl border border-[color:var(--portal-border)] bg-[color:var(--portal-card)] p-5 shadow-xl shadow-black/10">
    ...
    <div className="mt-4 max-h-[28rem] space-y-3 overflow-y-auto pr-1 portal-scrollbar">{loading ? <p className="py-10 text-center text-xs text-zinc-600"><Loader2 className="mr-2 inline animate-spin" size={14}/>Loading texts</p> : messages.length ? <AnimatePresence initial={false}>{messages.slice().reverse().map((message) => <motion.div layout initial={{ opacity: 0, y: 36, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.32, ease: [0.23, 1, 0.32, 1] }} key={message.id} className={`max-w-[88%] rounded-2xl px-4 py-3 ${message.direction === 'outbound' ? 'ml-auto bg-[#caa24c] text-black' : 'border border-[color:var(--portal-border)] bg-[color:var(--portal-soft)] text-zinc-200'}`}><p className="whitespace-pre-wrap text-sm">{message.body || '(Media message)'}</p><p className={`mt-1 text-[8px] font-black uppercase tracking-wider ${message.direction === 'outbound' ? 'text-black/55' : 'text-zinc-600'}`}>{message.direction === 'outbound' ? message.status : 'received'} · {new Date(message.created_at).toLocaleString()}</p></motion.div>)}</AnimatePresence> : <p className="py-10 text-center text-xs text-zinc-600">No texts with this client yet.</p>}</div>
    <div className="mt-4"><div className="flex gap-2"><textarea value={body} onChange={(event) => setBody(event.target.value)} disabled={!phone || sending} rows={3} placeholder={phone ? `Message ${contactName || phone}` : 'This lead has no mobile number'} className="min-w-0 flex-1 rounded-xl border border-[color:var(--portal-border)] bg-[color:var(--portal-soft)] px-3 py-2 text-sm text-white outline-none focus:border-[#caa24c]/40 disabled:opacity-50"/><button type="button" onClick={() => void send()} disabled={!phone || !body.trim() || sending} className="flex w-12 items-center justify-center rounded-xl bg-[#caa24c] text-black disabled:opacity-40">{sending ? <Loader2 size={16} className="animate-spin"/> : <Send size={16}/>}</button></div>{body.trim() ? <motion.p initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} className="mt-2 flex items-center gap-1 text-[9px] text-[#caa24c]">Typing <span className="flex gap-0.5">{[0,1,2].map((dot) => <motion.span key={dot} className="h-1 w-1 rounded-full bg-[#caa24c]" animate={{ y: [0,-3,0] }} transition={{ duration: 0.75, repeat: Infinity, delay: dot * 0.12 }}/>)}</span></motion.p> : null}</div>
  </section>
}
